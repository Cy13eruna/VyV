...
static func get_reachable_cells(
    ...
    while stack.size() > 0:
        ...
        for n_pos in nodes_dict[curr_pos].neighbors:
            ...
            # REGRA: Só é custo 0 se você JÁ ESTÁ em um nó aliado E vai para outro nó aliado.
            # Se você vem de fora (não aliado) para dentro, custa 1.
            var move_cost: int = 1
            if clean_allied.has(str_curr) and clean_allied.has(str_n):
                move_cost = 0
            else:
                move_cost = 1
            ...
            # Impedir atravessar unidades (Opcional, mas recomendado para evitar outros saltos)
            if clean_occupied.has(str_n):
                can_pass = false
            # Regra de Domínio/Terreno
            elif clean_allied.has(str_curr) or clean_allied.has(str_n):
                can_pass = true
            else:
