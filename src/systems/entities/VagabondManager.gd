...
# Referência ao Painter para evitar buscas repetitivas
...
func _on_turn_started(player_id: int, _player_color: Color) -> void:
	# Quando o turno começa, limpamos indicadores antigos do Painter
...
# --- LÓGICA DE INTERAÇÃO (CORREÇÃO DOS INDICADORES) ---

## Esta função deve ser chamada quando o jogador clica em uma unidade
func select_vagabond(v: Node2D) -> void:
	if not is_instance_valid(v) or not _painter: return
	
	# Capturamos os dados da unidade selecionada
	var pos = v.get("grid_pos")
	var m_range = v.get("move_range") if v.get("move_range") != null else 3
	var v_color = v.get("entity_color") # COR CRÍTICA AQUI
	
	# Se a cor falhar (null), usamos branco como fallback, nunca ciano
	if v_color == null: v_color = Color.WHITE
	
	# Calculamos os nós alcançáveis (ajuste conforme seu sistema de navegação)
	# Exemplo: var reachable = navigation.get_reachable(pos, m_range)
...
func _create_vagabond(grid_pos: Vector2, color: Color, player_id: int, grid: Node2D) -> void:
	...
	# Sincroniza posição mundial e grid
...
func _is_area_clear(pos: Vector2) -> bool:
	for v in active_vagabonds:
		if is_instance_valid(v):
			var v_grid_pos = v.get("grid_pos")
			if v_grid_pos is Vector2 and v_grid_pos.distance_to(pos) < 50.0: # Aumentado para segurança
				return false
	return true
