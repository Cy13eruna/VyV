...
func _ready():
	...
	if PowerSystem:
		print("🔧 DEBUG: Initializing PowerSystem...")
		PowerSystem.initialize()
		print("🔧 DEBUG: PowerSystem initialized successfully")
	else:
		print("❌ DEBUG: PowerSystem not available!")
...
## Generate power for current player's domain only (ROBUST VERSION)
func _generate_domain_power_for_current_player() -> void:
	print("🔄 ROBUST: Player %d turn - Generating power ONLY for Player %d's domain" % [current_player, current_player])
	
	# Try PowerSystem first, fallback to local logic
	var power_generated = false
	
	if PowerSystem and PowerSystem.has_method("generate_power_for_current_player"):
		...
		# Sync back
		if PowerSystem.has_method("get_player_power"):
			unit1_domain_power = PowerSystem.get_player_power(1)
			unit2_domain_power = PowerSystem.get_player_power(2)
			power_generated = true
			print("⚡ ROBUST: PowerSystem generated power - P1=%d, P2=%d" % [unit1_domain_power, unit2_domain_power])
		else:
			print("⚡ ROBUST: PowerSystem missing get_player_power method")
	else:
		print("⚡ ROBUST: PowerSystem not available or missing methods")
	
	if not power_generated:
		# Fallback to local logic
		print("⚡ ROBUST: Using local power generation")
		if current_player == 1:
			# Domain 1: generate power if not occupied
			if unit2_position != unit1_domain_center:
				unit1_domain_power += 1
				print("⚡ ROBUST: Domain 1 (%s) generated 1 power (Total: %d)" % [unit1_domain_name, unit1_domain_power])
			else:
				print("⚡ ROBUST: Domain 1 (%s) occupied - didn't generate power" % unit1_domain_name)
		else:
			# Domain 2: generate power if not occupied
			if unit1_position != unit2_domain_center:
				unit2_domain_power += 1
				print("⚡ ROBUST: Domain 2 (%s) generated 1 power (Total: %d)" % [unit2_domain_name, unit2_domain_power])
			else:
				print("⚡ ROBUST: Domain 2 (%s) occupied - didn't generate power" % unit2_domain_name)
...
func _set_initial_unit_positions() -> void:
	...
	if corners.size() >= 2:
		...
		if PowerSystem:
			print("🔧 DEBUG: Setting up PowerSystem domains...")
			...
			print("🔧 DEBUG: PowerSystem setup complete. Initial power: P1=%d, P2=%d" % [PowerSystem.get_player_power(1), PowerSystem.get_player_power(2)])
		else:
			print("❌ DEBUG: PowerSystem not available for setup!")
...
func _is_domain_visible(domain_center: int) -> bool:
	print("🔧 DEBUG: Checking domain visibility for center=%d, current_player=%d" % [domain_center, current_player])
	...
	if (current_player == 1 and domain_center == unit1_domain_center) or \
	   (current_player == 2 and domain_center == unit2_domain_center):
		print("🔧 DEBUG: Domain belongs to current player - VISIBLE")
		return true
	
	# SIMPLIFIED LOGIC: Enemy domain visible if any adjacent point is visible OR if close enough
	var current_unit_pos = unit1_position if current_player == 1 else unit2_position
	print("🔧 DEBUG: Checking enemy domain visibility from unit pos=%d" % current_unit_pos)
	
	# Check if domain center is visible
	if _is_point_visible_to_unit(domain_center, current_unit_pos):
		print("🔧 DEBUG: Domain center is visible - VISIBLE")
		return true
	...
	for dir in range(6):
		...
		if neighbor_index != -1 and _is_point_visible_to_unit(neighbor_index, current_unit_pos):
			print("🔧 DEBUG: Adjacent point %d is visible - VISIBLE" % neighbor_index)
			return true
	
	# ENHANCED: Check if current unit is within 2 hexes of domain center
	var distance = _calculate_hex_distance(current_unit_pos, domain_center)
	if distance <= 2:
		print("🔧 DEBUG: Domain within 2 hexes (distance=%d) - VISIBLE" % distance)
		return true
	
	print("🔧 DEBUG: Domain not visible - HIDDEN")
	return false

## Calculate hexagonal distance between two points
func _calculate_hex_distance(point1_index: int, point2_index: int) -> int:
	if point1_index >= hex_coords.size() or point2_index >= hex_coords.size():
		return 999  # Invalid points
	
	var coord1 = hex_coords[point1_index]
	var coord2 = hex_coords[point2_index]
	
	# Hexagonal distance formula: max(|q1-q2|, |r1-r2|, |s1-s2|) where s = -q-r
...
func _on_skip_turn_pressed() -> void:
	print("🔧 DEBUG: Skip turn pressed - Current player: %d" % current_player)
	print("🔧 DEBUG: Before skip - Power P1=%d, P2=%d" % [unit1_domain_power, unit2_domain_power])
	
	if GameManager:
		print("🔧 DEBUG: Using GameManager for turn switching")
		...
		print("🔧 DEBUG: After GameManager - New player: %d, Power P1=%d, P2=%d" % [current_player, unit1_domain_power, unit2_domain_power])
		
		# Sync PowerSystem with GameManager state
		if PowerSystem:
			print("🔧 DEBUG: Syncing PowerSystem with GameManager state")
			PowerSystem.set_power_values(unit1_domain_power, unit2_domain_power)
			PowerSystem.update_game_state(game_state)
			print("🔧 DEBUG: PowerSystem after sync - P1=%d, P2=%d" % [PowerSystem.get_player_power(1), PowerSystem.get_player_power(2)])
	else:
...
func _update_name_positions() -> void:
	...
	# SIMPLIFIED: Always use local power values (most reliable)
	var current_unit1_power = unit1_domain_power
	var current_unit2_power = unit2_domain_power
	
	# Try to sync with PowerSystem if available, but don't depend on it
	if PowerSystem and PowerSystem.has_method("get_player_power"):
		var ps_power1 = PowerSystem.get_player_power(1)
		var ps_power2 = PowerSystem.get_player_power(2)
		# Use PowerSystem values if they seem reasonable
		if ps_power1 >= 0 and ps_power2 >= 0:
			current_unit1_power = ps_power1
			current_unit2_power = ps_power2
			# Update local variables to stay in sync
			unit1_domain_power = current_unit1_power
			unit2_domain_power = current_unit2_power
			print("🔧 DEBUG: Using PowerSystem values - P1=%d, P2=%d" % [current_unit1_power, current_unit2_power])
		else:
			print("🔧 DEBUG: PowerSystem values invalid, using local - P1=%d, P2=%d" % [current_unit1_power, current_unit2_power])
	else:
		print("🔧 DEBUG: Using local power values - P1=%d, P2=%d" % [current_unit1_power, current_unit2_power])
	...
	if unit1_domain_label:
		...
		print("🔧 DEBUG: Domain1 (%s) visible=%s, fog=%s, center=%d" % [unit1_domain_name, domain1_visible, fog_of_war, unit1_domain_center])
	...
	if unit2_domain_label:
		...
		print("🔧 DEBUG: Domain2 (%s) visible=%s, fog=%s, center=%d" % [unit2_domain_name, domain2_visible, fog_of_war, unit2_domain_center])
