...
signal empty_area_clicked(position: Vector2)
...
func _handle_mouse_button(event: InputEventMouseButton, grid_data: Dictionary, units_data: Dictionary) -> void:
	...
	if clicked_point != -1:
		...
		# No point found - emit empty area click for cancellation
		empty_area_clicked.emit(click_position)
