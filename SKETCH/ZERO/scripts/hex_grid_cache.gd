...
func build_cache(force_rebuild: bool = false) -> void:
	...
	print("HexGridCache: Building cache...")
	...
	print("HexGridCache: Generated %d hex positions" % cached_hex_positions.size())
	cached_dot_positions = _geometry.calculate_all_dot_positions(cached_hex_positions, _config.hex_size)
	print("HexGridCache: Generated %d dot positions" % cached_dot_positions.size())
	...
	print("HexGridCache: Cache built in %.2f ms, %d elements" % [last_rebuild_duration, total_size])
	print("HexGridCache: Hex positions: %d, Dot positions: %d, Connections: %d" % [cached_hex_positions.size(), cached_dot_positions.size(), cached_connections.size()])
...
func _on_config_changed(property_name: String, old_value, new_value) -> void:
	print("HexGridCache: Configuration changed (%s), invalidating cache" % property_name)
	invalidate_cache()
...
func _build_spatial_grid() -> void:
	...
	print("HexGridCache: Building spatial grid with cell size: %.2f" % spatial_cell_size)
	...
	print("HexGridCache: Spatial grid built with %d cells" % spatial_grid.size())
...
func _build_connections_and_geometry() -> void:
	...
	if cached_dot_positions.is_empty():
		print("HexGridCache: No dot positions available for connections")
		return
	
	var connection_distance = _geometry.calculate_connection_distance(_config.hex_size)
	print("HexGridCache: Connection distance: %.2f" % connection_distance)
	var processed_connections = {}  # Avoid duplicate connections
	
	# Build connections between nearby dots
	print("HexGridCache: Starting connection building for %d dots" % cached_dot_positions.size())
	for i in range(cached_dot_positions.size()):
		if i >= cached_dot_positions.size():
			print("HexGridCache: Index %d out of bounds for dot_positions size %d" % [i, cached_dot_positions.size()])
			break
		...
		if i < 5:  # Debug first few dots
			print("HexGridCache: Dot %d at %s found %d nearby dots" % [i, dot_a, nearby_indices.size()])
