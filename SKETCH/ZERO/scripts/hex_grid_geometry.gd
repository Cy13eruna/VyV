...
func calculate_hex_positions(config) -> Array[Vector2]:
	...
	print("HexGridGeometry: Generated %d hexagons in hexagonal pattern (radius %d)" % [positions.size(), hex_radius])
	return positions
...
func calculate_all_dot_positions(hex_positions: Array[Vector2], hex_size: float) -> Array[Vector2]:
	...
	print("HexGridGeometry: Generated %d total dots (%d hex centers + %d unique vertices)" % [all_dots.size(), hex_positions.size(), all_dots.size() - hex_positions.size()])
	return all_dots
