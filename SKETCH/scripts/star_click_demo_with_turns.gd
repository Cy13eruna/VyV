...
func _ready() -> void:
	print("V&V: Inicializando sistema com turnos...")
...
func _setup_system() -> void:
	...
	print("V&V: Sistema com turnos ativado!")
...
func _setup_turn_system() -> void:
	...
	for unit in all_units:
		...
		print("Unidade adicionada ao grupo %s" % color_key)
	...
	print("Teams criados: %d" % teams.size())
	for team in teams:
		print("Team %s: %d unidades, %d domínios" % [team.name, team.units.size(), team.domains.size()])
...
func _update_turn_button() -> void:
	...
	print("Botão atualizado: %s (cor: %s)" % [current_team.name, current_team.color])
...
func _activate_team_turn(team_index: int) -> void:
	...
	if team_index < teams.size():
		...
		print("Turno %d - Team %s ativo (%d unidades)" % [current_turn, active_team.name, active_team.units.size()])
...
func _handle_left_click(global_pos: Vector2) -> void:
	...
	if movement_mode_active and selected_unit:
		...
		if clicked_star_id >= 0 and clicked_star_id in valid_movement_stars:
			...
			if not _can_unit_act(selected_unit):
				print("Unidade já usou sua ação neste turno!")
				return
	...
	if clicked_unit:
		...
		if not _is_unit_from_current_team(clicked_unit):
			print("Esta unidade não pertence ao seu team!")
			return
...
func _move_selected_unit_to_star(target_star_id: int) -> void:
	...
	if game_manager.move_unit_to_star(selected_unit, target_star_id):
		print("Unidade movida! Ações restantes: %d" % selected_unit.actions_remaining)
		
		# IMPORTANTE: Remover highlights após usar a ação
		if selected_unit.actions_remaining <= 0:
			print("Unidade sem ações - removendo highlights")
			_deactivate_movement_mode()
		else:
