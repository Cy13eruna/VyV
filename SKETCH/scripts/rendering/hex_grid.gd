...
func _initialize_components() -> void:
	...
	Logger.debug("Config created", "HexGrid")
	
	# Create geometry calculator
	geometry = HexGridGeometry.new()
	Logger.debug("Geometry created", "HexGrid")
...
func _on_config_changed(property_name: String, old_value, new_value) -> void:
	Logger.debug("Configuration changed - %s: %s -> %s" % [property_name, str(old_value), str(new_value)], "HexGrid")
	redraw_grid()
	grid_updated.emit()

## Handle cache invalidation
func _on_cache_invalidated() -> void:
	Logger.debug("Cache invalidated", "HexGrid")
	redraw_grid()

## Handle cache rebuild completion
## @param cache_size int: New cache size
func _on_cache_rebuilt(cache_size: int) -> void:
	Logger.debug("Cache rebuilt with %d elements" % cache_size, "HexGrid")
	redraw_grid()
		...
		if mouse_event.pressed:
			...
			if camera:
				...
				if mouse_event.button_index == MOUSE_BUTTON_WHEEL_UP:
					...
					Logger.debug("Zoom in to %.1fx" % new_zoom, "HexGrid")
...
func _center_grid_on_screen() -> void:
	...
	if camera:
		...
		Logger.debug("Camera centered at: %s with zoom: %s" % [camera.position, camera.zoom], "HexGrid")
	
	Logger.debug("Viewport size: %s" % viewport_size, "HexGrid")
	Logger.debug("Screen center: %s" % screen_center, "HexGrid")
	Logger.debug("Grid positioned at: %s" % position, "HexGrid")
...
func _can_process_input() -> bool:
	...
	if current_time - last_input_time < input_cooldown:
		Logger.debug("Input ignored (cooldown active)", "HexGrid")
		return false
	
	# Check if already rebuilding
	if is_rebuilding:
		Logger.debug("Input ignored (rebuild in progress)", "HexGrid")
		return false
...
func _increase_hexagon_count() -> void:
	...
	if grid_width >= max_grid_radius:
		Logger.warning("Maximum radius reached (%d). Cannot increase further for performance reasons." % max_grid_radius, "HexGrid")
		return
	...
	if grid_width >= performance_warning_radius:
		Logger.warning("Performance Warning! Radius %d = %d hexagons (~%d dots). Consider reducing for better performance." % [grid_width, hex_count, dot_count_estimate], "HexGrid")
	else:
		Logger.info("Increased radius to %d (%d hexagons, ~%d dots)" % [grid_width, hex_count, dot_count_estimate], "HexGrid")
...
func _decrease_hexagon_count() -> void:
	if grid_width > 1:  # Don't go below 1 (just center hexagon)
		...
		Logger.info("Decreased radius to %d (%d hexagons, ~%d dots)" % [grid_width, hex_count, dot_count_estimate], "HexGrid")
		...
		Logger.warning("Cannot decrease below radius 1 (minimum: 1 hexagon)", "HexGrid")

## Get dot positions for external use (e.g., StarMapper)
## @return Array[Vector2]: Array of dot positions
...
## @return Array[Vector2]: Array of hex center positions
func get_hex_positions() -> Array[Vector2]:
	if not is_initialized or not cache:
		return []
	return cache.get_hex_positions()

## Check if grid is ready for external use
...
func rebuild_grid() -> void:
	Logger.debug("Rebuilding grid...", "HexGrid")
	...
	Logger.debug("Grid rebuild complete", "HexGrid")
...
func _exit_tree() -> void:
	...
	Logger.debug("Cleaned up resources", "HexGrid")