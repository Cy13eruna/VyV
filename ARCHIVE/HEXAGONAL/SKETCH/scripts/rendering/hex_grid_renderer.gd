...
func _render_stars(canvas_item: CanvasItem) -> void:
	...
	for i in range(star_geometry.size()):
		...
		# FORCE WHITE COLOR FOR STARS - GUARANTEED
...
func _update_viewport(camera_transform: Transform2D) -> void:
	...
	if camera:
		...
		Logger.debug("Viewport updated - size: %s, camera pos: %s, zoom: %s" % [actual_size, camera_position, camera_zoom], "HexGridRenderer")
	else:
...
func optimize_for_performance(target_fps: float = 60.0) -> void:
	...
	if current_render_time > target_frame_time:
		...
		Logger.info("Performance optimization enabled (render time: %.2f ms)" % current_render_time, "HexGridRenderer")
		...
		Logger.debug("Quality increased (render time: %.2f ms)" % current_render_time, "HexGridRenderer")